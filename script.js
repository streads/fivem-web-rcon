var errorShow = false;
var tunnel = $("#tunnel").text();
var socket = io('http://sheepy.ovh:8080');
var lastcommand = "";

socket.on('connect', function () {
    socket.on('rcon_response', function(data){
        log(data.content)
    });
});
...
$("#connectbtn").on("click", function(){
    ...
    if (remoteInfo.length == 2 && patt.test(remoteInfo[0])){
        if(pass == ""){
            alert("error you need to enter a password")
        }else{
            socket.emit("rcon_connect", {ip: remoteInfo[0], port: remoteInfo[1], password: pass})
        }
    }else{
        alert("Bad hostname (example: 127.0.0.1:30120)")
    }
});
...
$('#commandInput').bind("enterKey",function(e){
    ...
    lastcommand = content
    if (content != "clear"){
        socket.emit("rcon_command", content)
    }else{
        $("#log").empty();
    }
    $('#commandInput').val("")
